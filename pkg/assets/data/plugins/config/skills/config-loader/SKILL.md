...
El cargador de configuración gestiona la lectura y parsing de archivos `.claude/hookify.*.local.md`. Extrae frontmatter YAML y cuerpo de mensaje para crear objetos `Rule` evaluables.
...
1. Buscar todos los archivos `.claude/hookify.*.local.md` con `glob`
