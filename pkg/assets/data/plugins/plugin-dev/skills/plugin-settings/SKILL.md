...
Los plugins pueden guardar configuración y estado en archivos `.claude/plugin-name.local.md` dentro del directorio del proyecto. Este patrón usa frontmatter YAML para configuración estructurada y contenido markdown para prompts o contexto adicional.

**Características clave:**
- Ubicación del archivo: `.claude/nombre-plugin.local.md` en raíz del proyecto
...
STATE_FILE=".claude/mi-plugin.local.md"
...
Crear `.claude/mi-plugin.local.md` en tu proyecto:
...
```bash
if [[ ! -f ".claude/mi-plugin.local.md" ]]; then
...
3. Añadir entrada gitignore para `.claude/*.local.md`
